...
import pickle as pk
from utils import *
from apps import dict_name2label
...
def task(filename):
    ...
    cond1 = os.path.isfile(os.path.join(pickle_dir, tail+'.pickle'))
    ...
    if (not cond1 and (tail in dict_name2label) == True):
        ...
        with open(os.path.join(pickle_dir, tail+'.pickle'), 'wb') as f:
            pk.dump((X, y), f)
...
if __name__ == '__main__':
    ...
    todo_list = gen_todo_list(pcap_dir)
